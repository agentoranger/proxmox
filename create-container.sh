...
main() {
    ...
    ct_memory=$(awk '/MemTotal/ {print int($2 / 4096)}' /proc/meminfo)  # Memory size:      Integer in MB (default = 1/4 of system memory)
    ct_cores=$(($(nproc) / 2))                                          # CPU Cores:        Integer (1 - 8192) (default = 1/2 of system cpus)
